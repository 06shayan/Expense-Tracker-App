...
function AddNewTransaction() {
  ...
  const handleFormSubmit = (e) => {
    ...
    if (text && amount) {
      const data = {id: uid,  backgroundColor: selectedButton === 'income' ? '#59CE8F' : '#FF0000' , item: text, amount: amount, date: new Date()};
  ...
  return (
    <>
      ...
      {error && text.length <= 0 ?
      <label id='validation'>This field can not be empty</label> : ""}
